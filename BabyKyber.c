...
int* AddVectors(int* a, int* b, int length) {
    int* res = (int*)malloc(sizeof(int) * length);  // Memory allocation

    if (res == NULL) {
        // Handle memory allocation failure (e.g. return null or handle error)
        return NULL;
    }

    for (int i = 0; i < length; i++) {
        res[i] = (a[i] + b[i]) % 17;
        if (res[i] < 0) res[i] += 17;  // Ensure non-negative results
    }
    ...
    for (int i = 0; i < length / 2; i++) {
      ...
      //  printf("After Reduction: \r\n");
       // PrintPoly(addRes, 7);
        ...
        // Correct memory copy
        memcpy(u[i], addRes, sizeof(int) * length);
        free(addRes);
...
void Decrypt(int s[][4], int u[][4], int v[4], int* res)
{
    ...
    // PolyDiv and Modulo17 transformations
    PolyDiv(s_by_u);
    Modulo17(s_by_u, s_by_u, 4);

    // Free temp1 and temp2 as we don't need them anymore
    free(temp1);
    free(temp2);

    // Subtract v from s_by_u to get the final result
    Sub(v, s_by_u, res, 4);

    // Free s_by_u after it has been used
    free(s_by_u);
